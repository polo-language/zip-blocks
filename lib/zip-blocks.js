...
// TODO: remove all parseOptions - now handled exclusively by cstor
// TODO: fix README.md as well!
...
function discoverZip(inPath, outPath, name, callback) {
  ...
  if (!this._filesOnly) {
    this.emit('error', new Error('Can only run exact with filesOnly = true'))
    return
  }
