...
function zipFilesInDir() {
  ...
  function listAndZip(inPath, outPath, name, callback) {
    ...
    function onFile(filePath, stats, statOverCB) {
      files[filePath] = { isDir: false , size: stats.size, }
      statOverCB()
    }

    function onDir(filePath, stat, statOverCB) {
      if (!that._filesOnly) {
        const thisFile = files[filePath] = { isDir: true, }
...
function discoverZip(inPath, outPath, name, callback) {
  ...
  function statTemps() {
    ...
    function onFile(filePath, stats, statOverCB) {
      ...
      files[originalPath] = { size: stats.size, isDir: false, }
...
function getBlocks(files) {
  ...
  if (this._addOversize) {
    ...
    const blocks = binPacker.firstFitDecreasing(files, 'size', max, false)
    const oversized = blocks.pop()
...
function doZip(blocks, outPath, name, callback) {
  ...
  for (const zipNum in blocks) {
    ...
    const thisBlock = blocks[zipNum]
    for (const key in thisBlock) {
